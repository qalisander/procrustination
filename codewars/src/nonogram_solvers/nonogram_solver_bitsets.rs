...
impl<const T: usize> Clues<T> {
    fn get_next_mandatory_bits(&self) -> BitVec {
        self.0
            ...
            .map(|clues| {
                matches!(
                    (clues.current_bit(), clues.next_bit()),
    ...
    fn get_next_banned_bits(&self) -> BitVec {
        self.0
            .iter()
            .map(|clues| matches!(clues.next_bit(), Some(EMPTY) | None))
            .collect()
    }
...
pub fn solve_nonogram<const T: usize>(
    ...
    if is_solved {
        return permutations_stack
            ...
            .collect_vec()
            .try_into()
            .unwrap(); // https://stackoverflow.com/questions/29570607/is-there-a-good-way-to-convert-a-vect-to-an-array
    } else {
    ...
    fn solve_nongram_rec<const T: usize>(
        ...
        for permutation in get_permutations::<T>(
            left_clues[current_permutation_index],
            next_mandatory_bits,
            next_banned_bits,
        ) {
...
pub(super) fn get_permutations<'a, const T: usize>(
    clues: &'a [u8],
    ...
    fn get_permutations_rec<'a, const T: usize>(
        ...
        let iter = (0..T + 1 - (init_offset + clues_sum + clues_borders))
            .filter_map(move |offset| {
                ...
                let has_zeroes_valid = next_banned_bits.is_disjoint(&permutation);
                ...
                if has_ones_valid && has_zeroes_valid {
                    Some(get_permutations_rec::<T>(
                        ...
                        next_mandatory_bits.clone(),
                        next_banned_bits.clone(),
                    ))
                } else {
                    None
                }
